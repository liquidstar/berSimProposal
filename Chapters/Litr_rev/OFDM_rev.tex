...
Where $A_{avg}^2 = 2\sigma_a^2$ for \gls{QAM} and $A_{avg}^2 = \sigma_a^2$ for \gls{MASK}. $\sigma_a^2$ is the amplitude variance of the $I$ or $Q$ channel signal of \gls{QAM} (only $I$ for MASK) over their signal constellation.
...
For $i=0$, $s_i(t) = A_i\cos\phi_i$ is a baseband signal which is equal to the $I$ channel data of \gls{QAM} or \gls{PSK}. Its \gls{PSD} is given by:
